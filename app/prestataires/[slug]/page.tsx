...
import {
  MapPin, ChevronLeft, ShieldCheck, Clock, CheckCircle2, HelpCircle, Share2,
} from "lucide-react";
...
export function generateMetadata({ params }: { params: { slug: string } }): Metadata {
  ...
  if (!vendor) return { title: "Prestataire introuvable" };
  return {
...
const FAQ = [
  { q: "Comment réserver ce prestataire ?", a: "Sélectionnez une prestation et une date, puis envoyez votre demande. Le prestataire vous répond avec un devis ; après validation, vous réglez l'acompte en ligne pour confirmer." },
  ...
  { q: "Puis-je personnaliser une prestation ?", a: "Oui. Indiquez vos besoins dans votre message : la plupart des prestataires Lystra proposent des formules sur mesure." },
];
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 pb-20 pt-6 sm:px-6 lg:px-8">
      ...
      <Link href="/prestataires" className="inline-flex items-center gap-1 text-sm text-lystra-gray transition hover:text-lystra-plum">
        <ChevronLeft className="h-4 w-4" /> Retour aux prestataires
      </Link>
      ...
      <div className="mt-8 grid grid-cols-1 gap-10 lg:grid-cols-[1fr_360px]">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            ...
            {vendor.homeService && <Badge variant="soft">Service à domicile</Badge>}
          </div>
          ...
          <section>
            ...
            {vendor.reviews && vendor.reviews.length > 0 ? (
              ...
              <p className="mt-4 text-sm text-lystra-gray">Aucun avis pour le moment — soyez le premier à recommander ce prestataire.</p>
            )}
          </section>
