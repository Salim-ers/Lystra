 ...
 * Layout volontairement asymétrique (1 grande + petites) pour éviter
 * l'effet grille générée par IA.
 */
      ...
      <div className="mt-10 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {FEATURED_EVENT_TYPES.map((m, i) => (
          <Link
            ...
            className={cn(
              "group relative overflow-hidden rounded-[1.5rem] border border-lystra-champagne/20 shadow-soft transition-all duration-300 hover:-translate-y-1 hover:shadow-card",
              i === 0 ? "sm:col-span-2 sm:row-span-2 lg:col-span-2" : "",
            )}
          >
            <div className={cn("relative", i === 0 ? "aspect-[16/10] sm:h-full sm:aspect-auto" : "aspect-[5/4]")}>
              <Image
                src={img(m.img)}
                alt={m.label}
                fill
